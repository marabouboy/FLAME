...
def EMPTYADJMTXFUNC(REF):
    ...
    for Flame_Emptyadjmtx_COUNT1 in range(Flame_Emptyadjmtx_REF1): #Simply create an empty adjecency matrix based on the number of elements as the input. This part is responsible for the correct number of columns.
    ...
    if CORRECTADJMTXINPUT != []:
        for Flame_Rightadjmtx_COUNT1 in CORRECTADJMTXINPUT: #Loop: Iterates through the input (Translated Correct Reads).
            ...
            for Flame_Rightadjmtx_COUNT2 in range(Flame_Rightadjmtx_EXONNR):
                ...
                Flame_Rightadjmtx_Counter1 = 0
                Flame_Rightadjmtx_Counter2 = 0
                for Flame_Rightadjmtx_COUNT2 in REF: #Loop: Crosschecks with the reference. If it matches, breaks out of the loop, otherwise, add to a counter to know which position in the Adjecency matrix it needs to add to.
...
def SPLICESIGNALFUNC(SPLICESIGNALINPUT, REF):
    ...
    for Flame_SpliceSignal_COUNT1 in Flame_SpliceSignal_SPLICESIGNALINPUT:
        Flame_SpliceSignal_GU = False
        Flame_SpliceSignal_AG = False
        Flame_SpliceSignal_WINDOW = REF[(int(Flame_SpliceSignal_COUNT1[0])-3):
