...
#
# Create a Transcriptome wide function? There is no way in creating a system that allows for the FLAME pipeline to be done
# on each gene, but maybe a flagging function that allows the user to know which genes they should proceed to analyze with
# using a combination of the Filter+Translate and the Quantify function?
#####################
...
parser.add_argument("--min", dest = "MINIMUM", help = "Variance Range", default = 10)
...
###FLAME: Whole Transcriptome Mode:
...
if args.GENE == "Transcriptome Mode": #<----------------- Change here
    ...
    for Flame_Main_COUNT1 in Flame_Main_GENELIST2:
        ...
        if len(Flame_Main_INPUT2) < Flame_Main_MINIMUMREADS:
            ...
            continue
        if Flame_Main_RATIO >= float(0.25):
...
#FLAME: Single-Gene Mode:
    ...
    #print(Flame_Main_TRANSLATEANNOTATED)#, Flame_Main_TRANSLATEINCONGRUENT)
    if args.VERBOSE:
        ##Print out Annotated:
